...
from utils import clean_option, validate_image_file, sanitize_filename, validate_roll_number, calculate_percentage, extract_marks_from_text, clean_multiple_options, validate_multiple_options, calculate_partial_marks, parse_options_string, extract_weightage_scheme
...
def init_database():
    ...
    try:
        ...
        try:
            ...
            # Check if marks_text column exists
            cursor.execute("""
                SELECT column_name 
                FROM information_schema.columns 
                WHERE table_name='correct_answers' AND column_name='marks_text';
            """)
            
            if not cursor.fetchone():
                # Add marks_text column to store original marking scheme text
                cursor.execute("ALTER TABLE correct_answers ADD COLUMN marks_text TEXT;")
                print("✅ Added marks_text column to correct_answers table")
...
@app.route('/api/answer-key-ocr', methods=['POST'])
def teacher_upload_ocr():
    ...
    try:
        ...
        try:
            ...
            # Additional check for empty correct options
            empty_questions = []
            for answer in ocr_result.get('answers', []):
                correct_options = answer.get('correct_options', [])
                if not correct_options or len(correct_options) == 0:
                    empty_questions.append(answer.get('question_number', 'unknown'))
            
            if empty_questions:
                return jsonify({
                    "error": f"OCR failed to detect correct answers for questions: {', '.join(map(str, empty_questions))}. Please ensure the answer key has clear markings (diagonal lines, circles, checkmarks, etc.) for each correct option.",
                    "suggestion": "Try uploading a clearer image or use manual answer key entry instead.",
                    "empty_questions": empty_questions
                }), 400
                
        except Exception as e:
            return jsonify({"error": f"OCR validation error: {str(e)}"}), 400
        ...
        try:
            ...
            for answer in ocr_result['answers']:
                ...
                if 'correct_options' in answer:
                    ...
                    try:
                        cursor.execute("""
                            INSERT INTO correct_answers (paper_id, question_number, correct_option, correct_options, marks, question_type, marks_text) 
    ...
    try:
        ...
        for answer in answers:
            ...
            if 'selected_options' in answer:
                ...
                selected_option = selected_options[0] if selected_options else 'NONE'  # Provide fallback for NOT NULL constraint
            else:
                # Old format
                selected_option = clean_option(answer.get('selected_option', '')) or 'NONE'  # Provide fallback
                selected_options = [selected_option] if selected_option and selected_option != 'NONE' else []
        ...
        try:
            ...
            cursor.execute("""
                SELECT ca.question_number, ca.correct_option, ca.correct_options, ca.marks, ca.question_type, ca.marks_text,
            ...
            cursor.execute("""
                SELECT ca.question_number, ca.correct_option, ca.marks, ca.marks_text,
                       sa.selected_option,
        ...
        for result in question_results:
            ...
            # Extract weightage scheme from marks_text if available
            weightage_scheme = None
            marks_text = result.get('marks_text', '')
            if marks_text:
                weightage_scheme = extract_weightage_scheme(marks_text)
                print(f"DEBUG Q{result['question_number']}: marks_text='{marks_text}', weightage={weightage_scheme}")
                print(f"DEBUG Q{result['question_number']}: correct_options={correct_options}, selected_options={selected_options}")
            
            # Use enhanced partial marking system with weightage support
            marking_result = calculate_partial_marks(selected_options, correct_options, result['marks'], weightage_scheme)
            print(f"DEBUG Q{result['question_number']}: marking_result={marking_result}")
...
def get_paper_answers(paper_id):
    ...
    try:
        ...
        try:
            # Try new schema first
            cursor.execute("""
                SELECT question_number, correct_option, correct_options, marks, question_type, marks_text 
            ...
            cursor.execute("""
                SELECT question_number, correct_option, marks, marks_text,
...
def get_detailed_results(submission_id):
    ...
    try:
        ...
        for row in rows:
            ...
            # Extract weightage scheme from marks_text if available
            weightage_scheme = None
            marks_text = row.get('marks_text', '')
            if marks_text:
                weightage_scheme = extract_weightage_scheme(marks_text)
                print(f"DEBUG DETAILED Q{row['question_number']}: marks_text='{marks_text}', weightage={weightage_scheme}")
            
            # Use enhanced partial marking system with weightage support
            marking_result = calculate_partial_marks(selected_options, correct_options, row['marks'], weightage_scheme)
...
@app.route('/api/enhanced-evaluate', methods=['POST'])
def enhanced_evaluate_sheets():
    """
    Enhanced evaluation endpoint for MCQ answer sheets
    Supports both equal marking and weightage marking modes
    """
    try:
        # Check for required files
        if 'teacher_image' not in request.files or 'student_image' not in request.files:
            return jsonify({"error": "Both teacher_image and student_image are required"}), 400
        
        teacher_image = request.files['teacher_image']
        student_image = request.files['student_image']
        
        # Validate image files
        if not validate_image_file(teacher_image):
            return jsonify({"error": "Invalid teacher image file"}), 400
        
        if not validate_image_file(student_image):
            return jsonify({"error": "Invalid student image file"}), 400
        
        # Process images with OCR
        teacher_image_data = ocr_processor.preprocess_image(teacher_image)
        student_image_data = ocr_processor.preprocess_image(student_image)
        
        if not teacher_image_data:
            return jsonify({"error": "Failed to process teacher image"}), 400
        
        if not student_image_data:
            return jsonify({"error": "Failed to process student image"}), 400
        ...
        teacher_valid, teacher_error = ocr_processor.validate_teacher_response(teacher_ocr_result)
        if not teacher_valid:
            return jsonify({"error": f"Teacher OCR validation failed: {teacher_error}"}), 400
        
        student_valid, student_error = ocr_processor.validate_student_response(student_ocr_result)
        if not student_valid:
            return jsonify({"error": f"Student OCR validation failed: {student_error}"}), 400
        
        # Initialize enhanced evaluator
        evaluator = EnhancedEvaluator()
        
        # Perform enhanced evaluation
        evaluation_results = evaluator.evaluate_from_images_data(teacher_ocr_result, student_ocr_result)
        ...
        detailed_evaluation = evaluator.evaluate_complete_sheet(teacher_data, student_data)
        ...
        return jsonify({
            ...
            "extracted_data": {
                "teacher": teacher_ocr_result,
                "student": student_ocr_result
            },
...
@app.route('/api/enhanced-evaluate-manual', methods=['POST'])
def enhanced_evaluate_manual():
    """
    Enhanced evaluation endpoint for manually provided data
    """
    try:
        data = request.get_json()
        ...
        if not teacher_data or not student_data:
            return jsonify({"error": "Both teacher_data and student_data are required"}), 400
        ...
        return jsonify({
            ...
            "message": "Manual enhanced evaluation completed successfully"
        })
        
    except Exception as e:
        return jsonify({"error": f"Manual evaluation failed: {str(e)}"}), 500

@app.route('/api/debug-ocr-extraction', methods=['POST'])
def debug_ocr_extraction():
    """Debug OCR extraction to see what's being extracted from teacher answer key"""
    try:
        if 'image' not in request.files:
            return jsonify({"error": "No image file provided"}), 400
        ...
        for answer in ocr_result.get('answers', []):
            ...
            if not correct_options or len(correct_options) == 0:
                empty_questions.append({
                    "question": q_num,
                    "marks_text": answer.get('marks_text', ''),
                    "marks": answer.get('marks', 1)
                })
            else:
        ...
        return jsonify({
            ...
            "ocr_result": ocr_result,
            "analysis": {
                "total_questions": len(ocr_result.get('answers', [])),
                "valid_questions": len(valid_questions),
                "empty_questions": len(empty_questions),
                "success_rate": f"{(len(valid_questions) / len(ocr_result.get('answers', [])) * 100):.1f}%" if ocr_result.get('answers') else "0%"
            },
            "valid_questions": valid_questions,
            "empty_questions": empty_questions,
            "recommendations": [
                "Ensure answer key has clear markings (diagonal lines, circles, checkmarks)",
                "Make sure image is well-lit and high resolution",
                "Avoid shadows or glare on the answer key",
                "Use dark pen/pencil for marking correct options",
                "Consider using manual answer key entry if OCR continues to fail"
            ]
        })
        
    except Exception as e:
        return jsonify({"error": f"Debug OCR failed: {str(e)}"}), 500

@app.route('/api/perfect-diagonal-detection', methods=['POST'])
def perfect_diagonal_detection():
    """Ultra-precise diagonal mark detection with detailed feedback"""
    try:
        if 'image' not in request.files:
            return jsonify({"error": "No image file provided"}), 400
        ...
        # Use ultra-precise prompt for diagonal detection with enhanced validation
        ultra_precise_prompt = """
        🎯 REVOLUTIONARY DIAGONAL MARK DETECTOR - ZERO ERROR TOLERANCE
        
        You are analyzing a teacher's MCQ answer key. Your mission: PERFECT accuracy with NO missed marks and NO false detections.
        
        ENHANCED DETECTION PROTOCOL:
        
        PHASE 1: QUESTION STRUCTURE ANALYSIS
        For each question (1-10):
        1. Identify the question number clearly
        2. List ALL available options in that specific question row
        3. Note: Questions may have different numbers of options (some have A-D, others A-F)
        
        PHASE 2: DIAGONAL MARK DETECTION
        For each available option:
        1. Look for diagonal lines that cross THROUGH the option letter
        2. Diagonal marks: /, \, X, or any stroke crossing the letter
        3. Ignore: circles, dots, underlines, or marks that don't cross through
        4. Certainty check: Only mark as correct if 100% certain there's a diagonal line
        
        PHASE 3: VALIDATION & ERROR PREVENTION
        1. ❌ NEVER include options that don't exist (e.g., option G in a question with only A-D)
        2. ❌ NEVER miss clear diagonal marks that cross through existing options
        3. ❌ NEVER mark options without clear diagonal lines
        4. ✅ Double-check each detection before finalizing
        
        PHASE 4: MARKS TEXT EXTRACTION
        1. Find handwritten marking text near each question
        2. Extract patterns: "Mark: 4", "mark: a=2, d=3, e=1", etc.
        3. Include exact text as found
        
        OUTPUT FORMAT (JSON):
        {
            "validation_report": {
                "total_questions_found": 10,
                "questions_analyzed": ["Q1", "Q2", "Q3", ...],
                "detection_confidence": "high"
            },
            "detailed_analysis": {
                "Q1": {
                    "available_options": ["a", "b", "c", "d"],
                    "diagonal_marks_detected": ["b"],
                    "marks_text": "Mark: 2",
                    "confidence": "high",
                    "notes": "Clear diagonal line crosses through option (b)"
                }
            },
            "final_answer_key": [
                {"question_number": 1, "correct_options": ["B"], "marks_text": "Mark: 2", "question_type": "single"}
            ]
        }
        
        CRITICAL SUCCESS CRITERIA:
        - 100% accuracy in option detection
        - Zero false positives or negatives
        - Only include options that actually exist
        - Never miss diagonal marks that clearly cross through options
        """
        
        # Get ultra-precise detection
        result = ocr_processor._process_with_gemini(image_data, ultra_precise_prompt)
        
        return jsonify({
            "success": True,
            "ultra_precise_result": result,
            "message": "Ultra-precise diagonal detection completed"
        })
        
    except Exception as e:
        return jsonify({"error": f"Perfect diagonal detection failed: {str(e)}"}), 500

@app.route('/api/universal-evaluate', methods=['POST'])
def universal_evaluate():
    """
    Universal MCQ evaluation endpoint using the new evaluation agent
    Expects JSON input with teacher_data and student_data
    """
    try:
        data = request.get_json()
        ...
        return jsonify({
            "success": True,
            "results": results,
            "message": "Universal MCQ evaluation completed successfully"
        })
        
    except Exception as e:
        return jsonify({"error": f"Universal evaluation failed: {str(e)}"}), 500
